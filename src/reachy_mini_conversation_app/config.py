...
class Config:
    ...
    # JETSON OPTIMIZATION
    # =========================================================================
    JETSON_OPTIMIZE = os.getenv("JETSON_OPTIMIZE", "true").lower().strip() in (
        "true",
        "1",
        "yes",
    )
    ...
    LLM_PROVIDER = (
        os.getenv("LLM_PROVIDER", "ollama" if JETSON_OPTIMIZE else "").lower().strip()
    )
    ...
    OLLAMA_MODEL = os.getenv(
        "OLLAMA_MODEL", "phi-3-mini-4k-instruct" if JETSON_OPTIMIZE else ""
    )
    ...
    if LLM_PROVIDER == "lmstudio":
        ...
        logger.info(
            f"LM Studio enabled at {LOCAL_LLM_ENDPOINT} with model {LOCAL_LLM_MODEL}"
        )
    elif LLM_PROVIDER == "ollama":
        LOCAL_LLM_ENDPOINT = OLLAMA_ENDPOINT
        LOCAL_LLM_MODEL = OLLAMA_MODEL or "llama3"
        logger.info(
            f"Ollama enabled at {LOCAL_LLM_ENDPOINT} with model {LOCAL_LLM_MODEL}"
        )
    elif LLM_PROVIDER:
        logger.warning(
            f"Unknown LLM_PROVIDER '{LLM_PROVIDER}'. Valid options: 'lmstudio', 'ollama'"
        )
    ...
    if _legacy_endpoint and not LLM_PROVIDER:
        ...
        logger.info(
            f"Local LLM enabled at {LOCAL_LLM_ENDPOINT} with model {LOCAL_LLM_MODEL} (legacy config)"
        )
    ...
    DISTIL_WHISPER_MODEL = os.getenv(
        ...
        if JETSON_OPTIMIZE
        else "distil-whisper/distil-medium.en",
    )
    WHISPER_LANGUAGE = os.getenv("WHISPER_LANGUAGE", "en")

    logger.info(
        f"Local ASR: Distil-Whisper ({DISTIL_WHISPER_MODEL}, {WHISPER_LANGUAGE})"
    )
    ...
    logger.info(
        f"Local TTS: Kokoro via FastRTC (voice: {KOKORO_VOICE}, speed: {KOKORO_SPEED})"
    )
    ...
    LOCAL_VAD_ENDPOINT = os.getenv(
        "LOCAL_VAD_ENDPOINT"
    )  # e.g., "http://192.168.68.74:7863"
